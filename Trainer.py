...
class Trainer:
    def __init__(self):
        ...
        self.strolling = 0
        self.pbar = tqdm(range(Config.epochs))
    ...
    def train(self):
        ...
        # statistic needs
        loss_record = []
        self.pbar.set_description(f"Train model_{Config.time_string}")
        for x_b, y_b in Config.train_loader:
            ...
            # Update Process Bar
            self.pbar.set_postfix({"train_loss": f"{loss:.3f}",
                                   "best_accuracy": f"{self.best_accuracy:.3%}",
                                   "strolling": self.strolling})
    ...
    def validate(self):
        ...
        correct_count = 0
        item_count = 0
        self.pbar.set_description(f"Valid model_{Config.time_string}")
        for x_b, y_b in Config.valid_loader:
            ...
            item_count += len(y_b)
            # Update process bar
            self.pbar.set_postfix({"valid_loss": f"{loss:.3f}",
